...
func (qw *QQWry) QueryIP(ip string) (areaInfo, extraInfo string) {
	...
	if len(country) > 1 && bytes.Compare(country[1:], cz88) != 0 {
		areaInfo, _ = iconv.ConvertString(string(country), "GB18030", "utf-8")
	}

	if len(area) > 1 && bytes.Compare(area[1:], cz88) != 0 {
		extraInfo, _ = iconv.ConvertString(string(area), "GB18030", "utf-8")
	}

	return areaInfo, extraInfo
}
